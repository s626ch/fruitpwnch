...
for (var i = 0; i < knownMessages.length; i++) {
    ...
    for(var j=0;j<scamExamples.length;j++){
        if(stringHandler.includes(scamExamples[j]) === true){
            knownMessages[i].innerHTML = '<div style="background-color:rgba(255,255,0,0.15);padding:12px;border-radius:8px;"><span style="color:var(--text-normal);">This message has been removed by FruitPwnch<br>due to it containing a scam,<br>or something else harmful to you or your computer.</span></div>';
        }
    }
}
